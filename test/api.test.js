...
describe('public API', () => {
  ...
  test('decodeToken round-trips through the options API', () => {
    ...
    const { token } = generateToken({ key, startingCode: STARTING_CODE, value: 7, count: 0, mode: constants.TOKEN_TYPE_SET_TIME });
  ...
  test('round-trips in restricted digit set mode', () => {
    const key = keyFromHex(OFFICIAL_KEY_HEX);
    const { token } = generateToken({ key, startingCode: STARTING_CODE, value: 3, count: 0, restrictedDigitSet: true });
    assert.strictEqual(token.length, 15);
    assert.match(token, /^[1-4]+$/);
    const r = decodeToken({ token, key, startingCode: STARTING_CODE, lastCount: 0, restrictedDigitSet: true });
  ...
  test('extended tokens round-trip through the options API', () => {
    const key = keyFromHex(OFFICIAL_KEY_HEX);
    const { token, newCount } = generateExtendedToken({ key, startingCode: EXT_STARTING_CODE, value: 123456, count: 0 });
